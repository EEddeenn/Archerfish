...
TEST_CASE("PM source produces constant amplitude", "[dsp][pm_source]") {
    PmSource src;
    src.configure({{"amplitude", 0.4},
                   {"carrier_freq_hz", 0.0},
                   {"mod_freq_hz", 1000.0},
                   {"mod_index", 2.0},
                   {"sample_rate", 1e6}});
    ...
    for (size_t i = 0; i < buf.size(); ++i) {
        REQUIRE_THAT(std::abs(buf[i]), WithinAbs(0.4f, 0.005f));
    }
}

TEST_CASE("PM source metadata reports Carson's bandwidth", "[dsp][pm_source]") {
    PmSource src;
    double mod_freq = 3000.0;
    double mod_index = 2.0;
    ...
    double expected_bw = 2.0 * (mod_index + 1.0) * mod_freq;
    REQUIRE_THAT(meta.nominal_bandwidth, WithinAbs(expected_bw, 1e-9));
}

TEST_CASE("PM source duration limit respected", "[dsp][pm_source]") {
    PmSource src;
    src.configure({{"amplitude", 0.2},
                   {"carrier_freq_hz", 0.0},
                   {"mod_freq_hz", 1000.0},
...
TEST_CASE("PM source reset produces identical output", "[dsp][pm_source]") {
    PmSource src;
...
TEST_CASE("PM source zero mod index produces constant phase", "[dsp][pm_source]") {
    PmSource src;
    src.configure({{"amplitude", 0.5},
                   {"carrier_freq_hz", 0.0},
                   {"mod_freq_hz", 1000.0},
    ...
    // With mod_index=0 and carrier=0, output should be constant
