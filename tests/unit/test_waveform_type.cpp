...
TEST_CASE("WaveformType round-trip to_string/waveform_type_from_string", "[waveform_type]") {
    const WaveformType all_types[] = {
        ...
        WaveformType::QAM16, WaveformType::QAM64, WaveformType::APSK16, WaveformType::APSK32,
        WaveformType::MultiTone, WaveformType::File,
        WaveformType::Pulse, WaveformType::ASK, WaveformType::FSK,
        WaveformType::AM, WaveformType::FM, WaveformType::PM, WaveformType::OFDM
    };
...
TEST_CASE("waveform_type_from_string accepts aliases", "[waveform_type]") {
    ...
    REQUIRE(waveform_type_from_string("64QAM").value() == WaveformType::QAM64);
    REQUIRE(waveform_type_from_string("PSK8").value() == WaveformType::PSK8);
    REQUIRE(waveform_type_from_string("16-apsk").value() == WaveformType::APSK16);
    REQUIRE(waveform_type_from_string("32_apsk").value() == WaveformType::APSK32);
    REQUIRE(waveform_type_from_string("multi-tone").value() == WaveformType::MultiTone);
}
...
TEST_CASE("waveform_type_from_string is case-insensitive", "[waveform_type]") {
    ...
    REQUIRE(waveform_type_from_string(" ofdm ").value() == WaveformType::OFDM);
}
...
TEST_CASE("waveform_type_cli_name returns correct names", "[waveform_type]") {
    ...
    REQUIRE(std::string(waveform_type_cli_name(WaveformType::APSK16)) == "16-APSK");
    REQUIRE(std::string(waveform_type_cli_name(WaveformType::APSK32)) == "32-APSK");
    ...
    REQUIRE(std::string(waveform_type_cli_name(WaveformType::OFDM)) == "OFDM");
}
